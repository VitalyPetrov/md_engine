...
void Results::print_snapshot(const m3Xf& input_positions, const m3Xf& input_velocities, int numParticles, int currentTimestep) {
  ...
  for (int i = 0; i < numParticles; i++) {
      ...
      vx = input_velocities(0, i);
      y = input_positions(1, i);
      vy = input_velocities(1, i);
      z = input_positions(2, i);
      vz = input_velocities(2, i);
      fout << i << "\t" << x << "\t" << y << "\t" << z << "\t" << vx << "\t" << vy << "\t" << vz << "\n";
  }
  fout.close();
}
